...
function put(id, callback) {
  setData(id, (doc) => {
    if (doc != 0) {dbConstans.MovieModel.update({id: id}, {$set: doc}, function (err) {
                if (err) {
                    console.log(id + "更新失败")
                } else {
                    console.log(id + "更新成功")
  ...
  dbQuery.findOneByID(id, function (doc) {
    if (doc != 0 && doc.files.length > 0) {
      ...
      for (let i = 0; i < doc.files.length; i++) {
        let data = doc.files[i].download;
        if (data != "" && data != undefined) {xunlei.transfer(doc.files[i].download, (url) => {
